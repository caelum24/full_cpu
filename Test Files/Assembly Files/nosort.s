...
addi $s3, $zero, 200    # $s3 = NUMVECTORS number of vectors needed to be created 
...
addi $t4, $t1, 411  # $t4 = location of next dot (811 total words in a dot -> moving to the next one)
...
bne $t0, $t1, is_dead #if reachedgoal != 1, go to is_dead
...
search: #a0 is head of linkedlist
 
addi $t0, $a0, 0 #t0 = current of linkedlist
lw $t1, 8($t0) #t1 = fitness
addi $t9, $t1, 0 #t9 = best fitness
addi $t8, $t0, 0 #t8 = location of best fitness
...
search_cycle:
lw $t0, 9($t0) #current = current.next
bne $t0, $zero search_loop

addi $v0, $t8, 0 #v0 = best location
# addi $v1, $t9, 0 #v1 = fitness
...
bne $a1, $a0, begin_mutate
j exit_mutate

begin_mutate:
addi $t0, $zero, 0 #counter for total vector location
addi $t1, $zero, 400 #NUMVECTORS*2
...
addi $s2, $zero, 70 #while counter < NUMDOTS, we loop move
...
# # SEARCHING THROUGH DOTS TO FIND LOCATION OF BEST ONE (CHAMPION)
add $a0, $s0, $zero #making $a0 the head of the linkedlist
add $s5, $s0, $zero #making $s5 head of linkedlist
jal search
add $s0, $v0, $zero #making s0 location of champion in list
...
add $a0, $s0, $zero #making $a0 the location of the champion
add $a1, $s5, $zero #making $a1 the head of the linkedlist
